...
package Mail::DKIM::Algorithm::rsa_sha512;
...
sub init_digests
{
	...
	# initialize a SHA-512 Digest
	$self->{header_digest} = new Digest::SHA(512);
	$self->{body_digest} = new Digest::SHA(512);
}
...
sub sign
{
	...
	my $signature = $private_key->sign_digest("SHA-512", $digest);
...
sub verify
{
	...
	return unless $public_key->verify_digest("SHA-512", $digest, $sig);
